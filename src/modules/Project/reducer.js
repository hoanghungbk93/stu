...
export const addProject = (header, params) => async dispatch => {
  ...
  try {
    fetch(`https://cc44e5d1.ngrok.io/api/studa/addnewda`, {
      method: 'POST',
...
export const deleteProject = (header, projectId) => async dispatch => {
  ...
  try {
    fetch(`https://cc44e5d1.ngrok.io/api/studa/deleteda/?id=${projectId}`,{
      method: 'DELETE',
    }).then((response) => {
      console.log('responseaa', response)
      if(!response.ok) throw new Error(response.status);
...
export const editProject = (header, params) => async dispatch => {
  try {
    fetch(`https://cc44e5d1.ngrok.io/api/studa/updateda`, {
      method: 'PUT',
      body: JSON.stringify(params),
      headers: {
        "Content-Type": 'application/json'
...
export const getListProject = (header, projectuserId) => async dispatch => {
  try {
    ...
    fetch(`https://cc44e5d1.ngrok.io/api/studa/getallda`).then((response) => {
